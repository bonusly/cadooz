class Cadooz::Immutable::Order
  ...
  def initialize(open_struct)
    ...
    @delivery_address = Cadooz::Address.new(open_struct&.delivery_address)
    @generation_profile = open_struct&.generation_profile
    @greeting_card = Cadooz::GreetingCard.new(open_struct&.greeting_card)
    @invoice_address = Cadooz::Address.new(open_struct&.invoice_address)
    @invoice_information = open_struct&.invoice_information&.each_with_object([]) { |i, arr| arr << Cadooz::InvoiceInformation(i) }
    ...
    @order_positions = open_struct&.order_positions&.each_with_object([]) { |o, arr| arr << Cadooz::OrderPosition(o) }
